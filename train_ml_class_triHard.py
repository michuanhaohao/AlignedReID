...
import models
from util.losses import CrossEntropyLoss, DeepSupervision, CrossEntropyLabelSmooth, TripletLoss, MetricMutualLoss
...
def main():
    ...
    model = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'softmax','metric'}, use_gpu=use_gpu)
    model2 = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'softmax', 'metric'},
                              use_gpu=use_gpu)
    ...
    criterion_metric = TripletLoss(margin=args.margin, mutual_flag=True)
    criterion_ml = MetricMutualLoss()
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(epoch, model, model2, criterion_class, criterion_metric, criterion_ml, optimizer1, optimizer2,trainloader, use_gpu)
...
def train(epoch, model, model2 ,criterion_class, criterion_metric, criterion_ml,optimizer, optimizer2, trainloader, use_gpu):
    model.train()
    losses = AverageMeter()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        if args.htri_only:
            if isinstance(features1, tuple):
                loss1, dist1 = DeepSupervision(criterion_metric, features1, pids)
                loss2, dist2 = DeepSupervision(criterion_metric, features2, pids)
            else:
            ...
            if isinstance(features1, tuple):
                htri_loss1,dist1 = DeepSupervision(criterion_metric, features1, pids)
                htri_loss2,dist2 = DeepSupervision(criterion_metric, features2, pids)
            else:
