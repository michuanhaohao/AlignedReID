...
import models
from util.losses import CrossEntropyLoss, DeepSupervision, CrossEntropyLabelSmooth, TripletLoss, TripletLossAlignedReID
...
def main():
    ...
    model2 = models.init_model(name=args.arch, num_classes=dataset.num_train_pids, loss={'softmax', 'metric'},aligned= True,use_gpu=use_gpu)
    ...
    criterion_metric = TripletLossAlignedReID(margin=args.margin, mutual_flag=True)
    ...
    for epoch in range(start_epoch, args.max_epoch):
        ...
        train(epoch, model, model2, criterion_class, criterion_metric, optimizer1, optimizer2,trainloader, use_gpu)
        ...
        if (epoch + 1) > args.start_eval and args.eval_step > 0 and (epoch + 1) % args.eval_step == 0 or (
            ...
            save_checkpoint({
                ...
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep' + str(epoch + 1) + 'model1.pth.tar'))
            save_checkpoint({
                'state_dict': state_dict2,
                'rank1': rank1,
                'epoch': epoch,
            }, is_best, osp.join(args.save_dir, 'checkpoint_ep' + str(epoch + 1) + 'model2.pth.tar'))
...
def train(epoch, model, model2 ,criterion_class, criterion_metric, optimizer, optimizer2, trainloader, use_gpu):
    model.train()
    losses = AverageMeter()
    ...
    for batch_idx, (imgs, pids, _) in enumerate(trainloader):
        ...
        outputs1, features1, local_fearture1 = model(imgs)
        outputs2, features2, local_fearture2 = model2(imgs)
        if args.htri_only:
            if isinstance(features1, tuple):
                loss1, dist1 = DeepSupervision(criterion_metric, features1, pids, local_fearture1)
                loss2, dist2 = DeepSupervision(criterion_metric, features2, pids, local_fearture2)
            else:
                loss1, dist1 = criterion_metric(features1, pids, local_fearture1)
                loss2, dist2 = criterion_metric(features2, pids, local_fearture2)
        else:
            ...
            if isinstance(features1, tuple):
                htri_loss1,dist1 = DeepSupervision(criterion_metric, features1, pids, local_fearture1)
                htri_loss2,dist2 = DeepSupervision(criterion_metric, features2, pids, local_fearture2)
            else:
                htri_loss1,dist1 = criterion_metric(features1, pids, local_fearture1)
                htri_loss2,dist2 = criterion_metric(features2, pids, local_fearture2)
        ...
        if (batch_idx+1) % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'MLoss {ml_loss.val:.4f} ({ml_loss.avg:.4f})\t'.format(
                   ...
                   data_time=data_time, loss=losses, xent_loss=xent_losses, htri_loss=htri_losses, ml_loss= mutual_losses))
